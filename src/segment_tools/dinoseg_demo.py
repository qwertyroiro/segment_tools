...
class DINOSeg:
    def __init__(
        ...
        self.groundingdino_model = load_model_hf(
            ckpt_repo_id, ckpt_filenmae, ckpt_config_filename, device
        )
        self.sam_predictor = SamPredictor(
            sam_model_registry[sam_checkpoint](checkpoint=sam_checkpoint_path).to(device)
        )
    ...
    def run(self, image, text):
        ...
        annotated_frame, detected_boxes = __detect(
            image, image_source, text_prompt=text, model=self.groundingdino_model
        )
        if len(detected_boxes) == 0:
            return None
        segmented_frame_masks = __segment(
