...
def suppress_output(suppress_stdout=True, suppress_logging=True):
    # 既存の標準出力と標準エラー出力を保存
...
with suppress_output():
    ...
    print("\ndinosegのテスト")
    annotated_frame_with_mask, segmented_frame_masks = st.dinoseg(image_path, prompt)
    cv2.imwrite("image_dir/dinoseg.png", annotated_frame_with_mask)
